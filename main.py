...
while request_for_command == True:
	...
	if command == '1':
		...
		url = input('YouTube-link (https://www.youtube.com/watch?v=...) or path to song: ')
		if url.endswith('.mp3'):
			song_name = url[:-4]
		else:
			'''
			...
			with YoutubeDL() as ydl:
				...
				os.rename(video_title+'-'+video_id+'.mp4', video_title+'.mp4')
		...
		text2 = translit(song_name, 'ru', reversed=True)
		print(text2)

		text3 = song_name.replace("'", '').replace('/', '').replace('\\', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '').replace('.', '').replace(',','').replace('#', '').replace('$', '').replace('—', '-')

		text = text2.replace("'", '').replace(' ', '').replace('/', '').replace('\\', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '').replace('.', '').replace(',','').replace('#', '').replace('$', '').replace('—', '-')
		if url.endswith('.mp3') == False:
			'''
			video = VideoFileClip(f"{text3}.webm")
			...
			path = directory+'/'+text3

			os.rename(f"{path}.webm", f"{path}.webm")

			subprocess.run('ffmpeg -i "'+path+'".webm "'+path+'".mp3',shell=True)
		...
		if url.endswith('.mp3'):
			os.rename(f"{url}", f"{text}.mp3")
		else:
			os.rename(f"{text3}.mp3", f"{text}.mp3")
